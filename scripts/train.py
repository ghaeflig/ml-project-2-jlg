...
def train_func(train_loader, model, epoch, criterion, optimizer, writer=None, scaler=None, device=DEVICE, print_err=TRUE) : #rajouter scaler if cuda
    model.train()
    train_loss = 0
    accuracies = 0
    writer = SummaryWriter()
    # batch_x = train images and batch_y = train masks
    for it, (batch_x, batch_y) in ennumrate(train_loader) :  #for batch_x, batch_y in train_loader :
    ...
    if print_err :
         print("\nEpoch {} | Train loss: {:.5f} and train accuracy: {:.5f}".format(epoch, train_loss, accuracy))
        


def train_val(val_loader, epoch, device = DEVICE, print_err=TRUE) :
    model.eval()
    accuracies = 0
    with torch.no_grad() :
    ...
    accuracy = accuracies/len(val_loader)      
    if writer is not None :
        writer.add_scalar("Validation accuracy / epoch", accuracy, epoch)
    if print_err :
        print("\nEpoch {} | Validation accuracy: {:.5f}".format(epoch, accuracy)
              
              
    # save a print overlay or not ??
...
def training() :
    ...
    writer = SummaryWriter(comment="") # folder location: runs/May04_22-14-54_s-MacBook-Pro.comment/
    ...
    for epoch in range(NUM_EPOCHS):
        max_accuracy = 0
        train_func(train_loader, model, epoch, criterion, optimizer, witer, scaler)
        accuracy = train_val(val_loader, epoch)
        #scheduler.step()
